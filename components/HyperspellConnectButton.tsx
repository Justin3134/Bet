...
function HyperspellLogo({ size = 14 }: { size?: number }) {
  return (
...
export function HyperspellConnectButton() {
  ...
  async function handleConnect() {
    ...
    try {
      ...
      if (sessionError || !sessionData?.accessToken) throw new Error('Not authenticated');
  ...
  return (
    <button
      ...
      disabled={loading}
      className="flex items-center gap-1.5 text-mono-xs text-[var(--accent)] hover:opacity-80 transition-opacity disabled:opacity-40"
    >
      <HyperspellLogo size={11} />
